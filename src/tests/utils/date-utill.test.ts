...
describe('dateToStringCriteria', () => {
  it('start date of day', () => {
    expect(dateToStringCriteria(date, true)).toEqual(moment(date).startOf('day').toISOString());
  });

  it('end date of day', () => {
    expect(dateToStringCriteria(date, false)).toEqual(moment(date).endOf('day').toISOString());
  });
});
...
describe('dateTimeToDateOnlyString', () => {
  it('date format', () => {
    expect(dateTimeToDateOnlyString(date)).toEqual(moment(date).format('YYYY-MM-DD'));
  });
});
...
describe('dateStringToTagCriteria', () => {
  it('date format', () => {
    ...
    expect(dateStringToTagCriteria(dateStr)).toEqual(d.format(DateFormat.DATE_FORMAT));
  });
});
...
describe('dateStringToMonthTagCriteria', () => {
  it('month format', () => {
    ...
    expect(dateStringToMonthTagCriteria(dateStr)).toEqual(d.format(DateFormat.MONTH_FORMAT));
  });
});
...
describe('getDateFromString', () => {
  it('get Date', () => {
    expect(getDateFromString(dateStr, YMD_FORMAT)).toEqual(moment(dateStr, YMD_FORMAT).toDate());
  });
});
...
describe('convertBkkToUtc', () => {
  it('bkk to utc', () => {
    ...
    let utcDate = datetime.add(-timeZone, 'hours').format(DateFormat.DATE_TIME_NONO_SEC);
    utcDate = `${utcDate}Z`;
...
describe('convertUtcToBkk', () => {
  it('utc to bkk', () => {
    ...
    expect(convertUtcToBkk(dateStr, DateFormat.DATE_TIME_FORMAT)).toEqual(bkkDate);
  });
});
...
describe('convertUtcToBkkDate', () => {
  it('convert bkk date', () => {
    ...
    const bkkDate = datetime.utcOffset(7).format(DateFormat.DATE_TIME_FORMAT);
    expect(convertUtcToBkkDate(dateStr, DateFormat.DATE_TIME_FORMAT)).toEqual(bkkDate);
  });
});
