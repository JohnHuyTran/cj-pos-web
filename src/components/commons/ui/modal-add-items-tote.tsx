...
export default function ModalAddItemsTote({
  open,
  onClose,
  requestBody,
}: Props): ReactElement {
  ...
  const itemsList = useAppSelector(
    (state) => state.searchAllItemsList.itemList,
  );
  ...
  if (searchItem)
    options =
      itemsList && itemsList.data && itemsList.data.length > 0
        ? itemsList.data
        : [];
  ...
  const handleAddItems = async () => {
    ...
    let result: any = [];
    if (itemsTote.length > 0) {
      const sumAddItemList = [...itemsList, ...itemsTote];
      var o: any = {};
    ...
    await dispatch(updateItemsToteState(result));
    setNewAddItemListArray([]);
    setSearchItem(null);
    setTimeout(() => {
