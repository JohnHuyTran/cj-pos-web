...
export default function AlertError({ open, onClose, textError, items }: Props): ReactElement {
  let item: any = [];
  for (let i = 0; i < items.length; i++) {
    item.push(<Box sx={{ textAlign: 'left', paddingLeft: '10px' }}>{items[i]}</Box>);
  }
    ...
    <Dialog
      ...
      <DialogContent sx={{ padding: '1em' }}>
        <DialogContentText
          ...
          <Box>{textError}</Box>
          <Box sx={{ textAlign: 'left', paddingLeft: '10px' }}>เลขที่เอกสาร:</Box>
          {item}
        </DialogContentText>
      </DialogContent>
