...
export default function ModalShowPDF({
  ...
  const base64WithoutPrefix = file.substr("data:application/".length);

  const pdfFile = file.substr(5, 15);
  const imgFile = file.substr(5, 5);
  ...
  return (
    <div>
      <Dialog open={open} maxWidth={false}>
        ...
        <div
          style={{ height: "3000vh", minWidth: "500px", textAlign: "center" }}
        >
          {imgFile === "image" && (
            <img src={file} style={{ minWidth: "200px" }} />
          )}
        </div>
      </Dialog>
