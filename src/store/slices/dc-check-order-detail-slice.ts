...
import { CheckOrderDetailResponse, ItemsState } from '../../models/dc-check-order-model';
...
export const featchorderDetailDCAsync = createAsyncThunk('orderDetailDC', async (id: string) => {
  try {
...
const dcCheckOrderDetailSlice = createSlice({
  ...
  extraReducers: (builer) => {
    builer.addCase(featchorderDetailDCAsync.pending, () => {
      ...
      builer.addCase(featchorderDetailDCAsync.fulfilled, (state, action: PayloadAction<any>) => {
...
export const { clearDataFilter, setItemId, setReloadScreen } = dcCheckOrderDetailSlice.actions;
